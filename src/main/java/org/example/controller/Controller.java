...
public class Controller {
    ...
    public static void gestisciUtentiCondivisi(ToDo todo, Component parentComponent) {
        try (Connection conn = ConnessioneDatabase.getConnection()) {
            RichiestaDAO dao = new RichiestaDAO(conn);
            List<String> utenti = dao.getUtentiCondivisi(todo.getId());
            ...
            if (username.equals(autore)) {
                ...
                JOptionPane.showMessageDialog(parentComponent, "Hai condiviso con:\n" + elenco);

                // Autore può rimuovere utenti
                ...
                if (scelta == JOptionPane.OK_OPTION) {
                    ...
                    if (utenteSelezionato != null) {
                        try (Connection conn2 = ConnessioneDatabase.getConnection()) {
                            ToDoDAO daoToDo = new ToDoDAO(conn2);
                            daoToDo.rimuoviPartecipazione(todo.getId(), utenteSelezionato);
                            todo.getListaUtenti().remove(utenteSelezionato);
                            JOptionPane.showMessageDialog(parentComponent, "Utente rimosso dalla condivisione.");
                        } catch (Exception e) {
                            e.printStackTrace();
                            JOptionPane.showMessageDialog(parentComponent, "Errore nella rimozione: " + e.getMessage());
                        }
                    }
                }
                ...
                if (utenti.isEmpty()) {
                    JOptionPane.showMessageDialog(parentComponent, "Nessun altro utente con cui è condiviso.");
                } else {
                    String elenco = String.join("\n", utenti);
